...
const FJ_TOKEN = Forgejo.Token("secret token")
const USER = "a user"
...
@testset "GitForge.jl" begin
    ...
    @testset "Rate limit headers" begin
        rl = GitForge.RateLimiter()

        # No rate-limit metadata at all.
        _, out = capture() do
            GitForge.rate_limit_update!(rl, HTTP.Response(200))
        end
        @test isempty(out)
        @test rl.remaining == 1
        @test rl.reset == 0

        # Valid headers update the limiter state.
        _, out = capture() do
            GitForge.rate_limit_update!(rl,
                HTTP.Response(200, ["X-RateLimit-Remaining" => "5", "X-RateLimit-Reset" => "123"]))
        end
        @test isempty(out)
        @test rl.remaining == 5
        @test rl.reset == 123

        # A partially missing header still warns.
        _, out = capture() do
            GitForge.rate_limit_update!(rl, HTTP.Response(200, ["X-RateLimit-Remaining" => "abc"]))
        end
        @test length(out) == 1
        @test rl.remaining == 5
        @test rl.reset == 123

        # Malformed header values still warn.
        _, out = capture() do
            GitForge.rate_limit_update!(rl,
                HTTP.Response(200, ["X-RateLimit-Remaining" => "abc", "X-RateLimit-Reset" => "456"]))
        end
        @test length(out) == 1
        @test rl.remaining == 5
        @test rl.reset == 123
...
user_id_type(::ForgejoAPI) = Integer
...
@testset "Forgejo.jl" begin
    api = ForgejoAPI(; token = FJ_TOKEN)
    @test api.token == FJ_TOKEN
    @test api.url == Forgejo.DEFAULT_URL
    test_api(api, auth = true)
end
...
const ENCODING_SAMPLES = [
    ...
    Forgejo.Repo => "{\"id\":1451184,\"owner\":{\"id\":828772,\"login\":\"karei\",\"full_name\":\"\",\"email\":\"karei@noreply.codeberg.org\",\"avatar_url\":\"https://codeberg.org/avatars/45f3cbe342d36420d05e6e92dc9ac9d9d0451fba70a8102c878be45cd551bdf7\",\"html_url\":\"https://codeberg.org/karei\",\"language\":\"\",\"is_admin\":false,\"last_login\":\"0001-01-01T00:00:00Z\",\"created\":\"2026-02-04T14:16:45+01:00\",\"restricted\":false,\"active\":false,\"prohibit_login\":false,\"location\":\"\",\"pronouns\":\"\",\"website\":\"\",\"description\":\"\",\"visibility\":\"public\",\"followers_count\":0,\"following_count\":0,\"starred_repos_count\":1,\"username\":\"karei\"},\"name\":\"DependencyAtlas.jl\",\"full_name\":\"karei/DependencyAtlas.jl\",\"description\":\"\",\"private\":false,\"html_url\":\"https://codeberg.org/karei/DependencyAtlas.jl\",\"url\":\"https://codeberg.org/api/v1/repos/karei/DependencyAtlas.jl\",\"ssh_url\":\"ssh://git@codeberg.org/karei/DependencyAtlas.jl.git\",\"clone_url\":\"https://codeberg.org/karei/DependencyAtlas.jl.git\",\"website\":\"\",\"stars_count\":1,\"forks_count\":0,\"watchers_count\":1,\"open_issues_count\":0,\"open_pr_counter\":0,\"default_branch\":\"master\",\"archived\":false,\"created_at\":\"2026-03-20T13:11:44+01:00\",\"updated_at\":\"2026-03-29T11:05:01+02:00\",\"permissions\":{\"admin\":false,\"push\":false,\"pull\":true}}",
    Forgejo.Comment => "{\"assets\":[],\"body\":\"Forgejo comment sample\",\"created_at\":\"2026-03-29T20:10:00+09:00\",\"html_url\":\"https://codeberg.org/karei/DependencyAtlas.jl/issues/4#issuecomment-1\",\"id\":1,\"issue_url\":\"https://codeberg.org/api/v1/repos/karei/DependencyAtlas.jl/issues/4\",\"original_author\":\"karei\",\"original_author_id\":828772,\"pull_request_url\":\"https://codeberg.org/api/v1/repos/karei/DependencyAtlas.jl/pulls/4\",\"updated_at\":\"2026-03-29T20:10:00+09:00\",\"user\":{\"id\":828772,\"login\":\"karei\",\"full_name\":\"\",\"email\":\"karei@noreply.codeberg.org\",\"avatar_url\":\"https://codeberg.org/avatars/45f3cbe342d36420d05e6e92dc9ac9d9d0451fba70a8102c878be45cd551bdf7\",\"html_url\":\"https://codeberg.org/karei\",\"language\":\"\",\"is_admin\":false,\"last_login\":\"0001-01-01T00:00:00Z\",\"created\":\"2026-02-04T14:16:45+01:00\",\"restricted\":false,\"active\":false,\"prohibit_login\":false,\"location\":\"\",\"pronouns\":\"\",\"website\":\"\",\"description\":\"\",\"visibility\":\"public\",\"followers_count\":0,\"following_count\":0,\"starred_repos_count\":1,\"username\":\"karei\"}}",
