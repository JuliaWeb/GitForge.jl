...
const CB_TOKEN = CodebergToken("secret token")
const USER = "a user"
...
user_id_type(::CodebergAPI) = Integer
...
@testset "Codeberg.jl" begin
    api = CodebergAPI(; token = CB_TOKEN)
    @test api.token == CB_TOKEN
    @test api.url == Codeberg.DEFAULT_URL
    test_api(api, auth = true)
end
